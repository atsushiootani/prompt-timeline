...
列は使った額の順に並び、見出しに合計が出る。一番高いセッションが一番左に来る。
点の面積がそのプロンプトの取り分。`size by` のボタンで **cost / tokens** を切り替えられる
（素の点に戻すこともできる）。

両方あるのは、**この 2 つが同じ順位にならない**から。キャッシュされた入力は入力単価の 1/10、
出力は 5 倍で課金されるので、1 トークンの重さが揃っていない。実際の 1 日で測ると、
1M トークンあたりの実効単価は **$0.33 〜 $1.37** まで開き、
**10 セッション中 5 つで順位が入れ替わった**。

**金額は API 換算であって請求額ではない。** Claude Code のサブスクリプションはトークン
課金ではないので、この数字は「同じ通信を API でやったらいくらか」でしかなく、
セッション同士を比べる以外の意味を持たない。だから常にトークン数を併記している
— 価格は変わるし新しいモデルも出るが、transcript にあるトークン数は事実として残るので、
価格表が古びても後から検算できる。価格の無いモデルはトークンだけ数えて
`unpriced_models` に出す。黙って 0 円にはしない。

> `src/pricing.mjs` の 1 時間キャッシュ書き込みの倍率は**まだ裏を取っていない**。
> 日合計が数％ぶれるが、セッションの順位は変わらない。
